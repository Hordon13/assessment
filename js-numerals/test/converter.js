...
const convert = function (num) {
  ...
  if (isNaN(num)) {
    ...
    throw new TypeError('input is not a number, but not an integer')
  }

  const len = Math.ceil(Math.log10(num + 1));
  if (len > 15) {
    throw new RangeError('too long number, limit: 15')
  }
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    if ((i === chunks.length - 1 && chunks.length > 1) || (chunks[i].toString().length === 3 && chunks.length === 1) && num !== 0) {
      result += 'and '
    }
